 ...
 *
 */
public class Melanie2 extends AbstractClassifier implements MultiClassClassifier{

	private static final long serialVersionUID = 1L;
	
	public ClassOption baseLearnerOption = new ClassOption("baseLearner", 'b',
            "OnlineBoosting Classifier to train.", Classifier.class, "meta.StreamingRandomPatches");
    ...
    public FileOption outputFileOption = new FileOption("outputFile", 'f',
            "Destination ARFF file.", "output.txt", "xls", true);
...
//		if(sn != 0) {
//			instTmp.deleteAttributeAt(0);
//		}//remove data source number before training and evaluating
		if(sn == 0) {
			instTmp.setValue(0, getVotesFromSource(instTmp));
		}
    	instTmp.dataset().setClassIndex(instTmp.numAttributes()-1);

    	if(!this.numberOfSource.contains(sn)) {
    		this.numberOfSource.add(sn);
    		this.generateNewBaseLearnerList(this.numberOfSource.indexOf(sn), instTmp);
			this.newBaseLeaner.trainOnInstance(inst);
    	}
...
//            break;
    ...
    protected void generateNewBaseLearnerList(int sn, Instance instance) {
    	Classifier baseLearner = ((Classifier) getPreparedClassOption(this.baseLearnerOption)).copy();
    	baseLearner.resetLearning();
		baseLearner.trainOnInstance(instance);
    ...
    protected int detectDrfit(int sn, Instance inst) {
        ...
        boolean prediction;
		if(this.correctlyClassifies(inst)) {
...
//			System.out.printf("1");
	...
	public int getVotesFromSource(Instance inst) {
		int targetNum = this.numberOfSource.indexOf(0);
...
//		instTmp.deleteAttributeAt(0); //remove data source number before training and evaluating
		...
		for(int idx = 0; idx < this.ensemble.size(); idx++) {
			if (idx == targetNum)
				continue;
			List<Classifier> baseLearnerList = this.ensemble.get(idx);
			for(Classifier baseLearner: baseLearnerList) {
				for(int k = 0; k < baseLearner.getSubClassifiers().length; k++) {
					int i = this.ensemble.indexOf(baseLearnerList);
					int j = baseLearnerList.indexOf(baseLearner);
		...
		return Utils.maxIndex(combinedVote.getArrayRef());
	}
...
//    	instTmp.deleteAttributeAt(0); //remove data source number before training and evaluating
		...
		if(this.evaluateModel.getNumOfExamplesReceived() == this.instanceSizeOption.getValue()) {
			try {
				this.output.toText(this.evaluateModel.getResults(), this.outputFileOption.getValue(), this.runTimesOption.getValue());
			} catch (IOException e) {
