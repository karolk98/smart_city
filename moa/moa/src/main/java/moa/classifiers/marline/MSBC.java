package moa.classifiers.marline;

import moa.classifiers.marline.MultiSourceInstance;
...
import moa.classifiers.marline.DoubleVector;
...
public class MSBC extends AbstractClassifier implements MultiClassClassifier {
    public ClassOption baseLearnerOption = new ClassOption("baseLearner", 'l',
            "Classifier to train.", Classifier.class,"meta.StreamingRandomPatches");
...
//        instTmp.dataset().deleteAttributeAt(0);
        instTmp.dataset().setClassIndex(instTmp.numAttributes()-1);
//        System.out.println("Num"+instTmp.numInputAttributes());
//        System.out.println("Num"+instTmp.numInputAttributes());
        MultiSourceInstance instance = new MultiSourceInstance((int)inst.value(0), instTmp);
//        MultiSourceInstance instance = new MultiSourceInstance(0, instTmp);
        ...
        int ddmLevel = ensemble.get(id).get(size - 1).detectDrift(instTmp);
        if (ddmLevel == DDM_OUTCONTROL_LEVEL) {
            System.out.println("Out " + instancesSeen);
            this.changeDetected++;
...
//                            Instance instExp = inst.copy();
                            Instance instExp = bl.equals(ensemble.get(id).get(size - 1))
                                    ? instTmp.copy() : mp.getMappedInstance(bl.getCentroid(), ensemble.get(id).get(size - 1).getCentroid(), instTmp);
                            ...
                            for (int i = 0; i < sub_classifiers.length; i++) {
                                ...
                                if (votes[i].sumOfValues() > 0.0) {
                                    ...
                                    lossw += votes[i].getValue((1 - (int)instTmp.classValue()));
                                    lossc += votes[i].getValue((int) instTmp.classValue());
                                }
                            }
...
//                            System.out.println(entry.getKey()+ " : "+ entry.getValue().indexOf(bl));
                            ...
                            bl.updatePerformance(votes, instTmp, lossc, lossw);
                        }
                    }
                }
...
//        ctVote = new DoubleVector();
        Instance instTmp = inst.copy();
//        System.out.println(instTmp.numInputAttributes());
//        if(instTmp.numInputAttributes()==3) {
//            instTmp.dataset().deleteAttributeAt(0);
//            instTmp.dataset().setClassIndex(instTmp.numAttributes() - 1);
//        }
            ...
            for (BaseLearner bl : entry.getValue()) {
                if (bl.getCentroid() != null) {
                    ...
                    Instance instExp = bl.equals(tc)
                            ? instTmp.copy() : mp.getMappedInstance(bl.getCentroid(), tc.getCentroid(), instTmp);
                    Classifier[] sub_classifiers = bl.getClassifier().getSubClassifiers();
